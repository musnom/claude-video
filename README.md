...
5. **Frames + transcript are handed to Claude.** The script prints frame paths with millisecond `t=MM:SS.mmm` markers and the transcript with timestamps. Claude `Read`s each frame in parallel — JPEGs render directly as images in its context.
